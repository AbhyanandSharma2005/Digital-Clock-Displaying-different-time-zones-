// Full IANA time zone list (183 entries)
const timeZones = [
...
// Helper for unique clock IDs
function generateID(tz) {
  return tz + '-' + Math.random().toString(36).substring(2,8);
}
...
addBtn.addEventListener('click', () => {
  ...
  const clockObj = { id: generateID(tz), tz };
  clocks.push(clockObj);
  renderClocks();
});

// Remove clock with animation
...
function renderClocks() {
  ...
  clocks.forEach(clock => {
    ...
    card.innerHTML = `
      <button class="remove-btn" title="Remove" aria-label="Remove clock">&times;</button>
      <div class="clock-title">${clock.tz.replace(/_/g, ' ')}</div>
      <div class="clock-time" id="clock-time-${clock.id}">--:--:--</div>
      <div class="clock-date" id="clock-date-${clock.id}"></div>
    `;
    // Remove button
    card.querySelector('.remove-btn').onclick = () => removeClock(clock.id);
